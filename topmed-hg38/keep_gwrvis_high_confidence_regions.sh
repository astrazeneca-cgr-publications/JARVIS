...
#SBATCH --mem=30G          # Request amount of memory
#SBATCH -t 24:0:0           # Request 48 hours runtime

input_dir=$1
dataset=$2	# topmed-hg38
...
if [ "$dataset" == "topmed-hg38" ]
then
echo	"[OK] Dataset: ${dataset}"
else
	echo "Please provide a valid value for the 'dataset' argument."
	echo "    - Accepted values: 'topmed-hg38'"
	printf "\n>> Expected call format: [sbatch] keep_gwrvis_high_confidence_regions.sh [input_dir] [dataset: topmed-hg38]\n"
	exit
fi
...
include_file=../genomic-high-confidence-regions-hg38/high_conf_genomic_regions.mindepth20.bed.gz 
