...
for resource in "${pathogenic_resources[@]}"; do
	intersectBed -a $full_score_ref_file -b ../../variant_annotation/${resource}/${resource}.pathogenic.bed > ${score}.${resource}_pathogenic.bed &
done


# > Benign variant-specific files
for resource in "${benign_resources[@]}"; do
	intersectBed -a $full_score_ref_file -b ../../variant_annotation/${resource}/${resource}.benign.bed > ${score}.${resource}_benign.bed &
done
