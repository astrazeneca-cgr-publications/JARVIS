...
class ScoreBenchmarking:
	...
	def preprocess_scores_per_class(self, lof_df, interg_df):
		...
		print('lof_df (after filtering by MAF):', lof_df.shape)
		print('interg_df (after filtering by MAF):', interg_df.shape)
...
if __name__ == '__main__':
	...
	if len(sys.argv) > 2:
		pathogenic_classes = [sys.argv[2]]
	else:
		#pathogenic_classes = ['lof', 'dup_lof', 'inv_span', 'dup_partial', 'utr', 'copy_gain', 'promoter', 'intronic']
		pathogenic_classes = ['utr']
	
	print('Pathogenic classes:', pathogenic_classes)
