...
single_nt_offset = int(sys.argv[2])
chrom = sys.argv[3]
...
gwrvis_dir = out_dir + '/gwrvis_scores/single_nt_offset_' + str(single_nt_offset)
...
for chr in chroms:
	...
	cur_gwrvis_file = gwrvis_dir + '/studres.chr' + chr + '.single_nt_offset_' + str(single_nt_offset) + '.txt'
	...
	df['win_middle'] = (df['start'].astype(int) + df['end'].astype(int) + 1) / 2
	df['win_middle'] = df['win_middle'].astype(int)
	df['start'] = df['win_middle'] + single_nt_offset
	df['end'] = df['start'] + 1
	...
	cur_out_fh = gwrvis_dir + '/gwrvis.chr' + str(chr) + '.genomic_coords.single_nt_offset_' + str(single_nt_offset) + '.bed'
...
full_gwrvis_out_file =  gwrvis_dir + '/full_genome.all_gwrvis.single_nt_offset_' + str(single_nt_offset) + '.bed'
