...
for gen_class in ccds utr lincrna ucne intergenic; do
	tail -n+2 $clinvar_feature_table_dir/full_feature_table.clinvar.bed | awk -v gen_class="$gen_class" '{if($6 == gen_class) print $0}' > $clinvar_feature_table_dir/full_feature_table.clinvar.${gen_class}.bed
done


# > Create BED files: full gwRVIS windows for CCDS only; region-specific BED files for all other non-coding regions
intersectBed -wa -a $clinvar_feature_table_dir/all_gwrvis_windows.bed -b $clinvar_feature_table_dir/full_feature_table.clinvar.ccds.bed | sort -k1,1 -k2,2n | mergeBed > $clinvar_feature_table_dir/ccds_gwrvis_windows.bed


for gen_class in ccds utr lincrna ucne intergenic; do
	cat $clinvar_feature_table_dir/full_feature_table.clinvar.${gen_class}.bed | cut -f1,2,3 > $clinvar_feature_table_dir/${gen_class}_gwrvis_win_subregions.bed
done
...
for overlap_class in intergenic utr; do
	...
	intersectBed -wa -a $clinvar_feature_table_dir/ccds_gwrvis_windows.bed -b $clinvar_feature_table_dir/${overlap_class}_gwrvis_win_subregions.bed |  intersectBed -wao -a stdin -b $clinvar_feature_table_dir/full_feature_table.clinvar.${overlap_class}.bed | awk '{print $1"\t"$2"\t"$3"\t"$7}' | mergeBed -c 4 -o distinct > $clinvar_feature_table_dir/${overlap_class}.overlap_with_ccds.bed
