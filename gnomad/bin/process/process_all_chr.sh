...
#SBATCH --cpus-per-task=22         # Request 10 CPUs (cores) on a single node 
...
do
        echo Processing chr: $i
	echo "./get_chr_table.sh $vcf_dir $i $KEEP_PASS_ONLY $FILTER_SEGDUP $FILTER_LCR $out_dir $population"
