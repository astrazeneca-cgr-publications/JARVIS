...
gnomad_dir="gnomad_3"
gnomad_high_cov_file="../${gnomad_dir}/coverage-files/high_cov_bed_files-min_depth${mindepth}/gnomad.whole_genome.high_cov.min_depth${mindepth}.bed"
...
echo "Replacing 'chr' with '' (empty string)"  
...
echo "Filter out UCSC Repeats and SegDup regions from gnomad cov file with depth >= $mindepth"
...
subtractBed -a $tmp_dir/sufficient_cov_minus_UCSC_repeats_and_segdups.bed.gz -b $speedseq_regions | gzip > high_conf_genomic_regions.mindepth${mindepth}.bed.gz
